...
From **C-Play version 2.4.0**, C-Play can publish its own video as an NDI source, so that other applications on the network — OBS Studio, a control room monitor, another media player, etc. — can receive it like any other NDI feed. This is the reverse direction of [OBS Studio + NDI -> C-Play](obs-ndi), where C-Play receives an external NDI source as a layer.
...
## Main video NDI output<a name="main-video"></a>
...
## Layer NDI output<a name="layer-output"></a>
...
## Master-only layers ("Sync" button)<a name="master-only"></a>
...
## Receiving C-Play's NDI output<a name="receiving"></a>
...
For the reverse direction — sending OBS content *into* C-Play as an NDI layer — see [OBS Studio + NDI -> C-Play](obs-ndi).