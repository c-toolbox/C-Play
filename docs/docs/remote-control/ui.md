...
C-Play supports 360 movies and images (2D and 3D). However, in the dome, the content shown from that video is usually only half of it. To create an interactive moment in the 360 video, or to show another side, there is a button to rotate the sphere around and also transition the sphere to another defined state (see [Grid/mapping settings](/settings/grid) to change this state).
