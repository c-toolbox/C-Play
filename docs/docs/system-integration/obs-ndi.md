...
> **The reverse direction:** From C-Play v2.4.0, C-Play can also *send* its main video or individual layers out as NDI sources that OBS (or any other NDI receiver) can pick up. See [C-Play -> NDI output](/system-integration/ndi-output).
...
The OBS scene can then be controlled from C-Play through REST/WebSocket commands. See [Launch apps and control things with C-Play](/system-integration/rest-troll-obs) for the OBS WebSocket setup.
